...
export default function Navbar() {
  ...
  const isActive = (pathname) => {
    return location.pathname === pathname ? 'underline text-orange-600  font-bold' : '';
  };
  const logout=()=>{
  ...
  return (
<nav className="block fixed w-screen px-4 py-2 mx-auto text-black bg-white border shadow-md rounded-b-xl border-white/80 bg-opacity-80 backdrop-blur-2xl backdrop-saturate-200 lg:px-8 lg:py-4" style={{ zIndex: "999" }}>      <div className={`container flex flex-col lg:flex-row ${isMenuOpen ? 'justify-center' : 'justify-between'} items-center mx-auto text-blue-gray-900`}>
        {/* Logo and Mobile Menu Button */}
        <div className="flex justify-around items-center w-full lg:w-auto">
          <img
            src={logo}
            alt="khelnepal logo"
            className="h-12 me-3 cursor-pointer"
        ...
        <div className={`lg:flex ${isMenuOpen ? 'block' : 'hidden'}`}>
          ...
          <li className={`flex items-center p-1 text-xl font-medium leading-normal text-blue-gray-900 transition-transform transform hover:text-orange-600 ${isActive('/')}`}>
        <Link to="/">Home</Link>
      </li>
      <li className={`flex items-center p-1 text-xl font-medium leading-normal text-blue-gray-900 transition-transform transform hover:text-orange-600 ${isActive('/venues')}`}>
        <Link to="/venues">Venue</Link>
      </li>
      <li className={`flex items-center p-1 text-xl font-medium leading-normal text-blue-gray-900 transition-transform transform hover:text-orange-600 ${isActive('/aboutus')}`}>
        <Link to="/aboutus">About Us</Link>
      </li>
      <li className={`flex items-center p-1 text-xl font-medium leading-normal text-blue-gray-900 transition-transform transform hover:text-orange-600 ${isActive('/contactus')}`}>

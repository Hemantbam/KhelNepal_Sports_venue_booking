...
const Register = () => {
  ...
  return (
    <section className="bg-gray-50 dark:bg-gray-900" style={{ backgroundImage: 'url(https://images.unsplash.com/photo-1606925797300-0b35e9d1794e?q=80&w=1481&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D)', backgroundSize: 'cover', backgroundPosition: 'center'}}>
      <div className="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
        <div className="w-full lg:w-1/3 backdrop-blur-lg bg-white/30 flex justify-center items-center rounded-md shadow-2xl">
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            <h1 className="text-xl font-bold leading-tight tracking-tight text-black md:text-2xl dark:text-black">Create an account</h1>
            <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
              ...
              <div>
                ...
                <input type="password" name="password" id="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full rounded-md border border-gray-400 p-2 text-sm" required="" />
              </div>
              <div>
              ...
              <div className="flex items-start">
                ...
                <div className="ml-3 text-sm">
                  <label htmlFor="terms" className="font-light text-gray-500 dark:text-gray-900">I accept the{' '}<Link className="font-medium text-primary-600 hover:underline dark:text-orange-500" to="#">Terms and Conditions</Link></label>
                </div>
              </div>
              {error && <div className="text-red-500 text-sm font-bold">{error}</div>}
              {success && <div className="text-green-500 text-sm font-bold">{success}</div>}
              <button type="submit" className="w-full relative flex justify-center items-center text-white bg-orange-600 hover:bg-orange-700 focus:ring-4 focus:outline-none focus:ring-orange-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-primary-600 dark:hover:bg-orange-700 dark:focus:ring-orange-800" disabled={loading}>
                {loading && (
              ...
              <p className="text-sm font-light text-gray-500 dark:text-gray-900">Already have an account?{' '}<Link to="/login" className="font-medium text-primary-600 hover:underline dark:text-orange-500">Login here</Link></p>
              <div className="mt-2 text-gray-700 text-center"><Link to="/" className="font-bold text-orange-500 hover:text-orange-600">Go Back To Home</Link></div>
            </form>
          </div>
        </div>
      </div>
